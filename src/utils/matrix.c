...
/* Converts the matrix the LP data type representing a CPLEX LP */
cplp_t* matrix_to_lp(matrix_t *A, matrix_t *b, matrix_t *c, char sense)
{
    int i, j, p, nz;
    cplp_t *lp;

    int non_zero = matrix_count_non_zero(A);
    lp = create_cplp(A->nrows, A->ncols, non_zero, sense);
    ...
    for (j = 0; j < A->ncols; ++j) {
        ...
        for (i = 0; i < A->nrows; ++i) 
            if (A->data[i][j] != 0) {
                ++nz;
                lp->matind[p] = i;
                lp->matval[p++] = A->data[i][j];
            }

        lp->matcnt[j] = nz;
        //printf("%d matbeg: %d matcnt: %d matind: %d matval: %lf\n", j, lp->matbeg[j], lp->matcnt[j], lp->matind[j], lp->matval[j]);
    }
